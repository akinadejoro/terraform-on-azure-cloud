...
export DB_HOSTNAME=${azurerm_mysql_flexible_server.mysql_server.fqdn}
export DB_PORT=3306
export DB_NAME=${azurerm_mysql_flexible_database.webappdb.name}
export DB_USERNAME="${azurerm_mysql_flexible_server.mysql_server.administrator_login}@${azurerm_mysql_flexible_server.mysql_server.fqdn}"
export DB_PASSWORD=${azurerm_mysql_flexible_server.mysql_server.administrator_password}
...
resource "azurerm_linux_virtual_machine_scale_set" "web_vmss" {
  ...
  depends_on = [azurerm_mysql_flexible_database.webappdb] 
