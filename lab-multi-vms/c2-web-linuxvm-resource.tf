...
resource "azurerm_linux_virtual_machine" "web_linuxvm" {
  ...
  name = "${local.resource_name_prefix}-${var.computer_name}-${each.key}"
  # computer_name = var.computer_name # "web-linux-vm" # Hostname of the VM (Optional)
  resource_group_name = azurerm_resource_group.rg.name
  location = azurerm_resource_group.rg.location 
  size = var.instance_size # "Standard_DS1_v2"
  admin_username = var.admin_username # "azureuser"
  network_interface_ids = [ azurerm_network_interface.web_linuxvm_nic[each.key].id ]
  admin_ssh_key {
    username = var.admin_username # "azureuser"
    public_key = file("${path.module}/ssh-keys/terraform-azure.pub")
  }
  os_disk {
    caching = "ReadWrite"
    storage_account_type = var.os_disc_storage_type # "Standard_LRS"
  }  
